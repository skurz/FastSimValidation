...
void TauAnalyzer_MiniAOD::bookHistos(DQMStore::IBooker & ibooker_)
{
  ...
  for(int i_particle = 0; i_particle < 4; ++i_particle){
    ...
    for(std::vector<std::string>::const_iterator i_shortName = tagNamesShort.begin(); i_shortName != tagNamesShort.end(); ++i_shortName){
      ...
      h_xPt_yPt[i_particle][histoID] = ibooker_.book2D("relPt_vs_truePt", "", 50,0.,500., 100,0.,2.);
      h_xPt_yEta[i_particle][histoID] = ibooker_.book2D("etaDiff_vs_truePt", "", 50,0.,500., 100,-0.5,0.5);
      h_xEta_yPt[i_particle][histoID] = ibooker_.book2D("relPt_vs_trueEta", "", 50,-5.,5., 100,0.,2.);
      h_xEta_yEta[i_particle][histoID] = ibooker_.book2D("etaDiff_vs_trueEta", "", 50,-5.,5., 100,-0.5,0.5);
