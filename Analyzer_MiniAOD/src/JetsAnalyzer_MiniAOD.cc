...
void JetsAnalyzer_MiniAOD::analyze(edm::Event const& e, edm::EventSetup const& eSetup)
{
  ...
  {
    if (i_recojet->pt() < ptThrJet_ || abs(i_recojet->eta()) > etaThrJet_) continue;

    h_PtvsrecoJet->Fill(i_recojet->pt());
    h_EtavsrecoJet->Fill(i_recojet->eta());

    const reco::GenJet* matchedGenJet = i_recojet->genJet();

    if(matchedGenJet){
      h_PtvsjetRelPtDiff->Fill(matchedGenJet->pt(), (i_recojet->pt()-matchedGenJet->pt())/matchedGenJet->pt());
      h_EtavsjetRelPtDiff->Fill(matchedGenJet->eta(), (i_recojet->pt()-matchedGenJet->pt())/matchedGenJet->pt());

      h_PtvsjetEtaDiff->Fill(matchedGenJet->pt(), i_recojet->eta()-matchedGenJet->eta());
      h_EtavsjetEtaDiff->Fill(matchedGenJet->eta(), i_recojet->eta()-matchedGenJet->eta());

      h_PtvsjetPhiDiff->Fill(matchedGenJet->pt(), i_recojet->phi()-matchedGenJet->phi());
      h_EtavsjetPhiDiff->Fill(matchedGenJet->eta(), i_recojet->phi()-matchedGenJet->phi());  
    }

    if(abs(i_recojet->partonFlavour()) == 5 || i_recojet->bDiscriminator("combinedInclusiveSecondaryVertexV2BJetTags") > 0.941)
    std::cout << (abs(i_recojet->partonFlavour()) == 5) << "; " << (i_recojet->bDiscriminator("combinedInclusiveSecondaryVertexV2BJetTags") > 0.941) << std::endl;

  }

  for (reco::GenJetCollection::const_iterator i_genjet = GenJetCollection->begin(); i_genjet != GenJetCollection->end(); ++i_genjet) 
  {
      if (i_genjet->pt() < ptThrJet_ || abs(i_genjet->eta()) > etaThrJet_) continue;
  ...
  // b Tagging
  const std::vector<std::pair<std::string,float>> discriVec= RecoJetCollection->begin()->getPairDiscri();
  ...
  for(std::vector<std::pair<std::string,float>>::const_iterator i_discriVec = discriVec.begin(); i_discriVec != discriVec.end(); ++i_discriVec){
    std::cout << i_discriVec->first << std::endl;
  }
...
void JetsAnalyzer_MiniAOD::bookHistos(DQMStore::IBooker & ibooker_)
{
  ...
  h_EtavsgenJet = ibooker_.book1D("EtavsgenJet","# genJets vs eta",10.,-5.,5.);

  h_PtvsrecoJet = ibooker_.book1D("PtvsrecoJet","# recoJets vs pt",50.,0.,500.);
  h_EtavsrecoJet = ibooker_.book1D("EtavsrecoJet","# genJets vs eta",10.,-5.,5.);
